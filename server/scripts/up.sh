...
if [ -f "$DOCKER_DIR/../docker/.env" ] && [ ! -f "$DOCKER_DIR/.env" ]; then
    echo "Legacy recorder configuration found. Follow server/docs/migration.md first." >&2
    exit 1
fi
cd "$DOCKER_DIR"
...
if [ ! -f config/cameras.yml ]; then
    ...
    echo "✓ Created server/config/cameras.yml from the template"
    NEEDS_EDIT=true
fi
...
if [ "$NEEDS_EDIT" = true ]; then
    ...
    echo "  server/.env                 — authentication settings, camera passwords, absolute DATA_DIR"
    echo "  server/config/cameras.yml   — one entry per camera"
    echo ""
    echo "Then run this script again."
    exit 0
fi

# Authentication is validated by the application. Reject the legacy placeholder.
if grep -q '^NVR_AUTH_MODE=basic$' .env && grep -Eq '^NVR_PASS=(change-me)?$' .env; then
    echo "Set a non-placeholder private Basic-auth password." >&2
    exit 1
fi
...
docker compose run --rm nvr-config
...
echo ""
echo "  Private Basic mode: log in with NVR_USER / NVR_PASS."
echo "  Session mode: use your configured HTTPS origin, not the HTTP listener."
echo ""
echo "⚠ Do NOT forward this port on your router. Use Tailscale for remote access."
echo ""
echo "Useful commands:"
echo "  docker compose -f server/compose.yml logs -f nvr    # follow the recorder"
echo "  ./scripts/nvr-up.sh                                 # re-run after editing cameras.yml"
echo "  docker compose -f server/compose.yml down           # stop everything"
echo ""
