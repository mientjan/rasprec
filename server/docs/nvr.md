# RaspRec NVR configuration

For public HTTPS/login and manual photo/video capture see [server setup](../README.md).
The Basic-auth setup below is for private networks only: explicitly set
NVR_AUTH_MODE=basic. AWS uses session authentication instead.
...
**`server/.env`** — every secret lives here, and it is gitignored.
...
**`server/config/cameras.yml`** — passwords are referenced as `${VAR}`, so this
file stays safe to commit or share.

```yaml
retention:
  continuous: 24h   # the 24/7 archive
  clips_days: 7    # motion clips, kept longer
...
motion clips can be kept for 7 days while the full archive rolls over every day,
...
```bash
docker compose -f server/compose.yml logs -f nvr | grep <camera>
```

Stream failures are logged without raw upstream output to avoid leaking credentials. `401` means the password in `.env` is wrong;
