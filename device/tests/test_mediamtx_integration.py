"""Opt-in Docker checks: RUN_DOCKER_TESTS=1 PYTHONPATH=server pytest tests/ -q.
...
def test_compose_limits(tmp_path):
    ...
    compose.write_text(
        (ROOT.parent / "server/compose.yml")
        .read_text()
        .replace('env_file: ["${NVR_ENV_FILE:-.env}"]', f"env_file: [{env}]")
    )
    data = json.loads(
        docker(
            "compose",
